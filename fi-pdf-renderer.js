(() => {
  ...
  function scenarioTable(ctx,r,y){
    ...
    const cols=[76,190,190,132,122],heads=['AGE','FI TARGET','PROJECTED PORTFOLIO','MONTHLY REQUIRED','CURRENT-PLAN FUNDING'];
  ...
  async function render(r){
    ...
    let end=statGrid(ctx,[{label:'Target FI age',value:`Age ${Math.round(s.targetAge)}`},{label:'FI target at selected age',value:compact(r.target.target)},{label:'Projected current-plan funding',value:pct(Math.min(1,r.target.funding))},{label:'Modelled FI timing',value:fiTiming},{label:'Monthly investment required from now',value:`${money(r.requiredMonthly)}/mo`},{label:'Additional monthly investment required',value:`${money(r.additionalMonthly)}/mo`}],360,3);
    const ry=end+18;card(ctx,M,ry,CW,66,r.additionalMonthly>0?C.amber:C.pale,r.additionalMonthly>0?C.amberLine:'#b9ddd8',10);P().text(ctx,'Additional monthly investment required',M+16,ry+39,{size:10.5,weight:850,color:r.additionalMonthly>0?'#704c00':C.tealDark});P().text(ctx,`${money(r.additionalMonthly)}/mo`,W-M-16,ry+41,{size:22,weight:900,color:r.additionalMonthly>0?'#704c00':C.tealDark,align:'right'});
